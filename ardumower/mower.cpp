...
Mower::Mower(){
  ...
  // ------- wheel motors -----------------------------
  motorAccel                 = 5000;      // motor wheel acceleration - only functional when odometry is not in use (warning: do not set too low)
  #if defined (ROBOT_ARDUMOWER)
  ...
  #else // ROBOT_MINI
		motorPowerMax              = 8.0;         // motor wheel max power (Watt)			
		motorSpeedMaxPwm           = 255;       // motor wheel max Pwm  (8-bit PWM=255, 10-bit PWM=1023)
  ...
  odometryUse                = 1;          // use odometry?
	  ...
	  odometryTicksPerRevolution = 1060;       // encoder ticks per one full resolution (without any divider)
		...
		wheelDiameter              = 60;        // wheel diameter (mm)
		odometryWheelBaseCm        = 18;         // wheel-to-wheel distance (cm)
